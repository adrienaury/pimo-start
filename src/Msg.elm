...
update msg model =
    case msg of
        Add field ->
            let
                ...
                newYaml =
                    Yaml.file newFields
            in
            ( { model | fields = newFields, test = decodeConfigOrNothing newYaml }, updateYaml newYaml )

        ChangeName i name ->
            let
                newModel =
                    updateFieldName model i name

                newYaml =
                    Yaml.file newModel.fields
            in
        ...
        ChangeCoherentWith i value ->
            let
                newModel =
                    updateFieldCoherentWith model i (List.filter isNotEmpty (List.map String.trim (String.split "," value)))
        ...
        ChangeAvoidCollisions i value ->
            let
                newModel =
                    updateFieldAvoidCollisions model
                        i
        ...
        ChangeRegexPattern i pattern ->
            let
                newModel =
                    updateFieldRegexPattern model i pattern
